...
import { Message } from "@/components/contexts/types/Messages";
...
const DropdownMessage = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      const unreadMessages = response.filter(
        (msg: Message) => msg.status === "Received"
      );
  ...
  const handleView = async (id: number) => {
    ...
    if (messageToUpdate) {
      if (messageToUpdate.status === "Seen") {
        // setSelectedMessage(messageToUpdate);
        // setOpen(true);
      } else {
        const updatedMessage = { ...messageToUpdate, status: "Seen" };

        const updated = await _updateMessageStatus(id, updatedMessage.status);
